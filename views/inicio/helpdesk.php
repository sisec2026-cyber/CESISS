...
// DataTransfer global que acumula los archivos
...
inputArchivo.addEventListener('change', () => {
  ...
  nuevos.forEach(file => {
    ...
    if (![...dataTransfer.files].some(f => f.name === file.name)) {
      dataTransfer.items.add(file);
    }
  });

  // Actualiza el input con todos los archivos acumulados
...
// Elimina archivo del DataTransfer
function eliminarArchivo(nombre) {
  const nuevos = new DataTransfer();
  Array.from(dataTransfer.files).forEach(file => {
    if (file.name !== nombre) {
      nuevos.items.add(file);
    }
  });

  dataTransfer.items.clear();
  Array.from(nuevos.files).forEach(f => dataTransfer.items.add(f));
  inputArchivo.files = dataTransfer.files;
  actualizarPreview();
}
</script>
